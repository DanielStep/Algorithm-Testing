...
#include "sorted_queries.h"

void sortedQuery1(FILE *fpHeap){
    ...
    while(fread(page, sizeof(char), page_size, fpHeap) == page_size){
        ...
        for (int i = 0; i < page->num_records; i++) {
            ...
            } else if (matchRegex(record->iucr, theftRegex) != 0 && foundFlag == 1){
                exitPaging = 1;
                break;
            }
        }
    ...
    printf("sortedQuery1: %d matches found\n", matchCount);
    ...
    printf("sortedQuery1 run time: %lu seconds\n", duration);
    
}


void sortedQuery2(FILE *fpHeap){
    struct timeval tv_start;
    gettimeofday(&tv_start, NULL);
    ...
    while(fread(page, sizeof(char), page_size, fpHeap) == page_size){
        ...
        for (int i = 0; i < page->num_records; i++) {
            ...
            if (matchRegex(record->id, idRegex) == 0){
                ...
                matchCount++;
            } else if (matchRegex(record->id, idRegex) != 0 && foundFlag == 1){
                exitPaging = 1;
                break;
            }
        }
    ...
    printf("sortedQuery2: %d matches found\n", matchCount);
    ...
    gettimeofday(&tv_end, NULL);
    unsigned long duration = tv_end.tv_sec - tv_start.tv_sec;
    printf("sortedQuery2 run time: %lu seconds\n", duration);
    
}


void sortedQuery3(FILE *fpHeap){
    struct timeval tv_start;
    gettimeofday(&tv_start, NULL);
    ...
    printf("sortedQuery3: %d matches found\n", matchCount);
    ...
    gettimeofday(&tv_end, NULL);
    unsigned long duration = tv_end.tv_sec - tv_start.tv_sec;
    printf("sortedQuery3 run time: %lu seconds\n", duration);
}


void sortedQuery4(FILE *fpHeap){
    struct timeval tv_start;
    gettimeofday(&tv_start, NULL);
    ...
    printf("sortedQuery4: %d matches found\n", matchCount);
    ...
    gettimeofday(&tv_end, NULL);
    unsigned long duration = tv_end.tv_sec - tv_start.tv_sec;
    printf("sortedQuery4 run time: %lu seconds\n", duration);
}
